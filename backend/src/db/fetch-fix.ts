...
const SUFFIX: Record<string, string> = { gpu: 'graphics card', cpu: 'cpu processor', ram: 'ram memory', psu: 'power supply', case: 'pc case', mobo: 'motherboard' };
const REJECT_URL =
...
const modelToken = (t: string) =>
  glue(t.replace(VENDOR_WORDS, ' ').replace(/\b\d+\s?gb\b/gi, ' ').replace(/\bddr\d\b/gi, ' ').replace(RAM_SPEEDS, ' '));

interface DdgResult { title: string; image: string; width?: number; height?: number }

async function search(query: string): Promise<DdgResult[]> {
  const tk = await fetch(`https://duckduckgo.com/?q=${encodeURIComponent(query)}&iax=images&ia=images`, { headers: { 'User-Agent': UA } });
...
function hostRank(u: string): number {
  ...
  if (/asset\.msi|asrock|amd\.com|nvidia|corsair|seasonic|kingston|nzxt|deepcool|coolermaster|lian-?li|fractal|gskill|crucial|zotac|palit|xfx|sapphire|pny|gigabyte|asus/i.test(h)) return 2;
  return 3;
}
...
async function run(): Promise<void> {
  ...
  for (const slug of slugs) {
    ...
    if (!info) { process.stderr.write(`?? ${slug} — немає в сідері\n`); continue; }
    const token = modelToken(info.title);
    const query = `${info.title} ${SUFFIX[info.type] ?? ''}`.trim();
    let results: DdgResult[] = [];
    try { results = await search(query); } catch (e) { process.stderr.write(`ERR ${slug}: ${e}\n`); }

    // Кандидати: валідний тип+хост; заголовок містить токен (для RAM токен короткий — не блокуємо).
    const cands = results
      .filter((c) => /^https:\/\//.test(c.image) && /\.(jpe?g|png|webp)(\?|$|&)/i.test(c.image) && !REJECT_URL.test(c.image))
    ...
    if (process.env.LIST === '1') {
      ...
      cands.slice(0, 10).forEach((c, i) =>
        process.stderr.write(`  [${i}] ${c.title.slice(0, 60).padEnd(61)} ${new URL(c.image).host}\n    ${c.image}\n`),
      );
      continue;
    }
    ...
    const pickIdx = Object.fromEntries(
      (process.env.PICK ?? '').split(',').filter(Boolean).map((s) => s.split(':')),
    )[slug];
    ...
    for (const c of ordered.slice(0, 12)) {
      ...
      if (d) {
        ...
        for (const e of ['jpg', 'jpeg', 'png', 'webp']) { const f = path.join(destDir, `${slug}.${e}`); if (fs.existsSync(f)) fs.unlinkSync(f); }
        fs.writeFileSync(path.join(destDir, `${slug}.${d.ext}`), d.buf);
        process.stderr.write(`OK  ${slug.padEnd(26)} ${new URL(c.image).host} (${Math.round(d.buf.length / 1024)}KB)\n`);
        saved = true;
        break;
      }
      await sleep(300);
    }
    if (!saved) process.stderr.write(`--  ${slug.padEnd(26)} (жоден кандидат не завантажився; token=${token})\n`);
    await sleep(1200);
  }
}
